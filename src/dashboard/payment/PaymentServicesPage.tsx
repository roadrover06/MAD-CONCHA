...
const PaymentServicesPage: React.FC<PaymentServicesPageProps> = ({
  ...
  const handleAddPayment = async () => {
    try {
      ...
      // If any required field is missing, show error and return early
      ...
      // Referrer is optional: Only add if referrerId is set and not empty
      let referrerObj: PaymentRecord["referrer"] = undefined;
      if (form.referrerId && form.referrerId !== "") {
        // Try to find in employees list (with name property)
      ...
      if (payingRecordId) {
        ...
        // Decrease chemicals stock if service has chemicals and now paid
        await decreaseChemicalsStock(form.serviceId, form.variety);
        // Add points if this is a registered loyalty customer and now paid
        ...
        // Decrease chemicals stock if paid
        if (!payLater) {
          await decreaseChemicalsStock(form.serviceId, form.variety);
          await addLoyaltyPoints(customerName, plateNumber, 0.25);
        }
  ...
  const handleRowClick = (record: PaymentRecord) => {
    if (record.paid) {
      ...
      // Unpaid: open process payment dialog directly
      setForm({
